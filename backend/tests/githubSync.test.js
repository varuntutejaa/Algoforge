...
require.cache[prismaPath] = {
    ...
    filename: prismaPath,
    loaded: true,
    exports: { prisma: { user: { update: async (args) => { updates.push(args); return {}; } } } }
};
...
test('does nothing when sync is paused, even with a stored token', async () => {
    const { syncAcceptedSolution } = stubGithub(async () => { throw new Error('should not be called'); });
    const user = connectedUser({ githubSyncEnabled: false });
    assert.equal(await syncAcceptedSolution({ user, ...submission }), null);
});
